...
int main(int argc, char *argv[]) {
    ...
    // split start block in half for each process to sort
    ...
    printf("starting---\n");
    if((pid = fork()) > 0){
	// parent process close write pipe, sorts left block and reads right block from child
        close(p[1]);
        merge_sort(&l_block);
        read(p[0], r_block.first, r_block.size*sizeof(int));
        merge(&l_block, &r_block);
        printf("---ending\n");
        printf(is_sorted(data, size) ? "sorted\n" : "not sorted\n");
    } else {
	// child process closes read pipe, sorts right block and writes it to parent
        close(p[0]);
        merge_sort(&r_block);
        write(p[1], r_block.first, r_block.size*sizeof(int));
    }
    free(data);
    exit(EXIT_SUCCESS);
}
